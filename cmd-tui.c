				...
				struct tui_graph *target)
{
	...
	xasprintf(&str, "=== Function Call Graph for '%s' (session: %.*s) ===",
		  root_node->name, SESSION_ID_LEN, graph->ug.sess->sid);
	...
	/* special node */
	root = append_graph_node(&graph->ug.root, "========== Back-trace ==========");
	...
	/* special node */
	root = append_graph_node(&graph->ug.root, "========== Call Graph ==========");
			      ...
			      struct tui_graph *graph, bool full_redraw)
{
	...
	while (count < LINES - 2) {
		...
		if (is_special_node(&node->n)) {
			...
			if (!is_special_node(&node->n))
				width += snprintf(buf, sizeof(buf), "%d", node->n.nr_calls) + 4;
		}
...
static void print_report_header(struct ftrace_file_handle *handle, struct tui_report *report)
{
	attron(COLOR_PAIR(C_HEADER) | A_BOLD);
...
static void print_report_footer(struct ftrace_file_handle *handle, struct tui_report *report)
{
	char buf[COLS + 1];
...
static void print_report_field(struct tui_report *report, struct tui_report_node *node)
{
	printw("  ");
